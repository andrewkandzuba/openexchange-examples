...
public class RegistryController {
    ...
    public ResponseEntity add(@Valid @RequestBody RegistryPayload payload, Errors errors) throws IOException, PushWooshResponseException {
        if (errors.hasErrors()) {
    ...
    public ResponseEntity remove(@Valid @RequestBody RegistryPayload payload, Errors errors) throws IOException {
        if (errors.hasErrors()) {
            return ResponseEntity.badRequest().body(ValidationErrorBuilder.fromBindingErrors(errors));
        }
        try {
